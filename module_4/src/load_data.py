...
def normalize_record(record: dict[str, Any], p_id: int) -> dict[str, Any] | None:
    ...
    return {
        ...
        "us_or_international": record.get("student_type") or record.get("us_or_international"),
        "gpa": clean_float(record.get("gpa")),
        "gre": clean_gre(record.get("gre_score") or record.get("gre"), 130, 170),
        "gre_v": clean_gre(record.get("gre_v_score") or record.get("gre_v"), 130, 170),
        "gre_aw": clean_gre(record.get("gre_aw"), 0, 6),
        "degree": record.get("degree"),
        "llm_generated_program": record.get("llm-generated-program") or record.get("llm_generated_program") or record.get("program_name_raw"),
        "llm_generated_university": record.get("llm-generated-university") or record.get("llm_generated_university") or record.get("university_raw"),
    }


def create_applicants_table(database_url: str | None = None, *, drop_existing: bool = False) -> None:
...
def insert_applicants(records: Iterable[dict[str, Any]], database_url: str | None = None) -> int:
                ...
                cur.execute(
                    ...
                    WHERE NOT EXISTS (
                        ...
                        WHERE COALESCE(program, '') = COALESCE(%(program)s, '')
                          AND COALESCE(date_added, DATE '1900-01-01') = COALESCE(%(date_added)s, DATE '1900-01-01')
...
if __name__ == "__main__":
    count = load_initial_data()
    print(f"Loaded {count} applicant records into the applicants table.")
